...
export const HeroSection = () => {
  ...
  const adSlides = [
    {
      ...
      badge: "EXCLUSIVO",
      image: "https://images.unsplash.com/photo-1605152276897-4f618f831968?w=1200",
    },
    {
      id: "organizadores",
  ...
  const allSlides = [
    ...adSlides.map((ad) => ({ type: "ad" as const, ...ad })),
  ...
  const goNext = useCallback(() => setCurrentIndex((prev) => (prev + 1) % allSlides.length), [allSlides.length]);
  const goPrev = useCallback(() => setCurrentIndex((prev) => (prev - 1 + allSlides.length) % allSlides.length), [allSlides.length]);
    ...
    <section className="max-w-[1400px] mx-auto px-6 lg:px-8 pt-8 pb-4">
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-8">
        ...
        <div className="relative overflow-hidden rounded-[40px] border border-white/[0.06] min-h-[520px] lg:min-h-[600px] flex items-end group">
          ...
          {allSlides.length > 1 && (
            <>
              <button onClick={goPrev} className="absolute left-4 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-black/40 backdrop-blur-xl border border-white/10 flex items-center justify-center text-white hover:bg-black/60 transition-all opacity-0 group-hover:opacity-100">
                <ChevronLeft size={20} />
              </button>
              <button onClick={goNext} className="absolute right-4 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-black/40 backdrop-blur-xl border border-white/10 flex items-center justify-center text-white hover:bg-black/60 transition-all opacity-0 group-hover:opacity-100">
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 flex gap-2">
              {allSlides.map((_, i) => (
                <button
                  key={i}
                  onClick={() => goTo(i)}
                  className={`w-2.5 h-2.5 rounded-full transition-all ${i === currentIndex ? "bg-primary w-8" : "bg-white/30 hover:bg-white/50"}`}
                />
              ))}
            </div>
          )}
          ...
          <div className="relative z-10 p-10 lg:p-16 w-full max-w-2xl">
            <span className="inline-flex items-center px-4 py-2 rounded-full bg-primary/10 border border-primary/20 text-primary text-xs font-bold uppercase tracking-[0.15em] mb-6">
              {slide.type === "ad" ? slide.badge : slide.event?.min_price > 0 ? "Boletos disponibles" : "Proximamente"}
            </span>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="glass-card overflow-hidden relative flex-1 min-h-[280px]">
              {featuredEvent.image_url && (
                <div className="absolute inset-0">
                  <Image src={featuredEvent.image_url} alt={featuredEvent.name} fill className="object-cover opacity-30" sizes="(max-width: 768px) 100vw, 33vw" />
                </div>
              )}
              <div className="absolute inset-0 bg-gradient-to-br from-primary/[0.08] to-secondary/[0.06]" />
              <div className="relative z-10 p-8 h-full flex flex-col justify-between">
                <div>
                  <span className="badge badge-hot text-xs">DESTACADO</span>
                  <h3 className="text-xl font-black mt-4">{featuredEvent.name}</h3>
                ...
                <div className="flex items-center justify-between mt-6">
                  <div>
                    {featuredEvent.min_price > 0 ? (
                      <>
                        <p className="text-xs text-muted-dark">Desde</p>
                        <p className="text-2xl font-black text-primary">${featuredEvent.min_price.toLocaleString("es-MX")}</p>
                      </>
                    ) : (
                      <p className="text-sm text-muted-dark">Consulta disponibilidad</p>
                    )}
                  </div>
                  <Link
            ...
            <div className="glass-card p-8 relative overflow-hidden">
              ...
              <div className="relative z-10">
                <h3 className="text-lg font-black">Noticias osmi</h3>
                  ...
                  {events.slice(0, 3).map((e, i) => (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-semibold truncate group-hover:text-primary transition-colors">{e.name}</p>
